...
export function ConversationGenerator({ items, initialFavorites }: { items: ConversationItem[]; initialFavorites: string[] }) {
  ...
  const filtered=items.filter(item=>item.situation.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className="grid gap-6 lg:grid-cols-[.72fr_1.28fr]">
    <aside className="app-card h-fit p-5"><label className="relative block"><Search size={17} className="absolute left-3 top-3.5 text-muted"/><input aria-label="Buscar conversaciones" className="field pl-10" value={query} onChange={e=>setQuery(e.target.value)} placeholder="¿Sobre qué necesitas hablar?"/></label><div className="mt-4 max-h-[62vh] space-y-2 overflow-y-auto pr-1">{filtered.map(item=><button key={item.id} onClick={()=>choose(item)} className={`w-full rounded-xl border p-3 text-left text-sm font-medium transition ${selected?.id===item.id?"border-[#B85C42] bg-[#fff4ee]":"border-[#ddd3c6] bg-[#fffdf8] hover:bg-[#f4eee4]"}`}>{item.situation}</button>)}</div></aside>
    {selected?<article className="surface p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#74836B]">Guía de conversación</p><h2 className="editorial-title mt-2 text-3xl sm:text-4xl">{selected.situation}</h2></div><button disabled={pending} onClick={()=>startTransition(async()=>{const value=await toggleFavorite("conversation",selected.id);setFavorites(current=>{const next=new Set(current);if(value)next.add(selected.id);else next.delete(selected.id);return next;});})} className="grid size-10 place-items-center rounded-xl border border-[#ddd3c6]" aria-label="Guardar conversación"><Heart size={18} fill={favorites.has(selected.id)?"currentColor":"none"} className={favorites.has(selected.id)?"text-[#B85C42]":""}/></button></div>
      ...
      <section className="mt-7 rounded-2xl bg-[#e4e9df] p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-[#547055]">Una apertura posible</p><p className="mt-3 text-lg leading-8 text-[#354436]">“{selected.opening}”</p></section>
